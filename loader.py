...
class loader():
    ...
    #TO DO : add below the path to corpus-webis-editorial-quality-18.json
    def load_corpus(self):
        ...
        path_corpus = root + 'conll18-news-editorials-quality-data/corpus-webis-editorial-quality-18.json'
    ...
    def get_normalized_training_data(self, normalizing_method="sqrt"):
        ...
        df_train, _  = machine_learning.clip_outliers(df_train, df_test =None, lower_percentile=1,  upper_percentile=99)

        X_train_df = df_train.select_dtypes(include=[np.number])

        y_train_df = df_train[['liberal_majority', 'conservative_majority']]
        X_train_df, _  = machine_learning.normalize(X_train_df, None, normalizing_method="sqrt")
    ...
    def _apply_set_split_label(row, df_features):
        ...
        if aid not in df_features.index:
            ...
            row['conservative_majority'] = discourse_row['conservative_majority']
    ...
    def load_discourse_adus(self):
        ...
        discourse_adus_df = pd.read_json('data/discourse_adus_annotations.json', orient='records')
    ...
    def load_saved_pars_df(self):
        self.load_data_with_features()
        pars_features_df = pd.read_json('data/paragraphs_with_adu_liwc_lexicons.json', orient='records')
        ...
        if 'split_label' not in pars_features_df.columns.values:
            ...
            pars_features_df = pars_features_df.apply(loader._apply_set_split_label, args=(self.df_features,), axis=1)
        if 'discourse_level' not in pars_features_df.columns.values:
            print('setting discourse_level')
            pars_features_df = pars_features_df.apply(loader._apply_discourse_level, args=(pars_features_df,), axis=1)
        self.pars_features_df = pars_features_df
        return self.pars_features_df

    def load_paragraphs_with_features(self, content=False):
        self.load_data_with_features()
        pars_features_df = pd.read_json('data/pars_features.json', orient='records')
        pars_features_df.set_index('paragraph-id', inplace=True)
        print(pars_features_df.columns)
        


        pars_adus_df = pd.read_json('data/paragraphs_adus_annotations.json', orient='records')
        pars_adus_df.set_index('id', inplace=True)
        pars_features_df = pars_features_df.join(pars_adus_df)
        print('"pars_features_df" is set. Each row is a paragraph of the article with features')
        pars_features_df = pars_features_df[pars_features_df['dismiss'] == 'No']
        keep_cols = [c for c in pars_features_df.columns.values if c.startswith('liwc_') or c.startswith('adu_')]
        keep_cols.extend(['id','index', 'conservative_majority', 'liberal_majority', 'order-in-discourse',
        'mpqa_par_sentences_obj', 'mpqa_par_sentences_subj'])
        pars_features_df = pars_features_df[keep_cols]
        pars_features_df = pars_features_df.apply(loader._apply_set_split_label, args=(self.df_features,), axis=1)
        pars_features_df = pars_features_df.apply(loader._apply_discourse_level, args=(pars_features_df,), axis=1)
        if content:
            df_ = pd.read_json('../acl2019/data/pars_liwc.json', orient='records')
    ...
    def apply_add_majority(row):
        ...
        effect_num = np.argmax(results) # if equal, returns 0 which is considered challenging
    ...
    def get_explanation_subcorpus(self, save=False):
        self.corpus_explanation = self.corpus[[ 'annotator_id', 'article_id', 'effect','explanation']]
        if save:
            fname = 'corpus/corpus_explanation.csv'
            if os.path.isfile(fname) :
                print("Warning! File already exists. Deleting and recreating:", fname)
                os.remove(fname)
                self.corpus_explanation.to_csv('corpus/corpus_explanation.csv', encoding='utf-8')
    ...
    def get_ideologies_dfs(self):
        ...
        self.conservative = self.corpus[self.corpus['political_pole'] == 'conservative']
    ...
    def get_train_test_data(self, train_percent = 0.8, has_validation_data = True, add_to_corpus=False, article_ids = None):
        ...
        print('rounded training data: ', len(train_article_ids))
